...
WITH skill_struct AS (SELECT STRUCT_PACK(
        skill := 'python',
        type := 'programming'
) AS s
)
...
SELECT
    skill,
    job_title_short,
    MEDIAN(salary_year_avg) AS median_salaray
FROM flat_skills
WHERE job_title_short = 'Data Engineer'
GROUP BY skill, job_title_short
ORDER BY median_salaray DESC
LIMIT 20;
...
-- From the persepective of a Data Analyst, analyze the median salary per type of skill
...
FROM flat_skills
--WHERE job_title_short = 'Data Engineer'
GROUP BY skill_type, skill_name, job_title_short
ORDER BY median_salaray DESC
LIMIT 20;